...
plot!(X_test,stupred,ribbon=nsig.*sqrt.(stupred_cov),title="StudentT Regression",lw=lw,color=1,lab="",fillalpha=falpha,linecolor=2)
...
plot!(X_test,gppred,ribbon=nsig*gppred_cov,title="Gaussian Regression",lw=lw,color=1,lab="",fillalpha=falpha,linecolor=2)
plot!(X,f,color=:black,lw=lwt,lab="")
plot!(X,f.+nsig*σ,lab="",color=:black,lw=lwt,linestyle=:dash)
plot!(X,f.-nsig*σ,lab="",color=:black,lw=lwt,linestyle=:dash)

plaplace = scatter(X,y_laplace,lab="")
plot!(X_test,lappred,ribbon=nsig*sqrt.(lappred_cov),title="Laplace Regression",lw=lw,color=1,lab="",fillalpha=falpha,linecolor=2)
plot!(X,f,color=:black,lw=lwt,lab="")
plot!(X,f.+nsig*β*sqrt(2),lab="",color=:black,lw=lwt,linestyle=:dash)
plot!(X,f.-nsig*β*sqrt(2),lab="",color=:black,lw=lwt,linestyle=:dash)

phetero = plot(X,y_hetero,t=:scatter,lab="")
plot!(X_test,lappred,ribbon = nsig.*sqrt.(hetpred_cov),title="Heteroscedastic Regression",lw=lw,color=1,lab="",fillalpha=falpha,linecolor=2)
plot!(X,f,color=:black,lw=lwt,lab="")
plot!(X,f.+nsig*sqrt.(inv.(AGP.logistic.(g))),lab="",color=:black,lw=lwt,linestyle=:dash)
plot!(X,f.-nsig*sqrt.(inv.(AGP.logistic.(g))),lab="",color=:black,lw=lwt,linestyle=:dash)
