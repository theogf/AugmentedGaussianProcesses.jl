...
t_stoch = @elapsed stochmodel = OMGP.SparseXGPC(X,y,Stochastic=true,BatchSize=40,Autotuning=false,VerboseLevel=2,m=N_indpoints,noise=noise,kernel=kernel)
...
p#rintln("Sparse model : Acc=$(acc_sparse), time=$t_sparse")
