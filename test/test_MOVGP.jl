...
m = AGP.MOVGP(xs,_fs,transform(SqExponentialKernel(),5.0),GaussianLikelihood(),AnalyticVI(),2)
# m.A[1][1] = [1.0, 0.0]
# m.A[2][1] = [0.0, 1.0]
AGP.get_K(m)
train!(m, 100)
pred = first.(first(predict_f(m, xs)))
nplus = 100
Xplus = range(10, length = nplus, step = Float64(X.step))
arpred = AGP.predict_ar(m, p, nplus, y_past = _fs)
...
m = AGP.MOVGP(xs,_ys,transform(SqExponentialKernel(),0.1),GaussianLikelihood(),AnalyticVI(),3,optimiser=false)
# m.A[1][1] = [1.0, 0.0, 0.0]
# m.A[2][1] = [0.0, 1.0, 0.0]
# m.A[3][1] = [0.0, 0.0, 1.0]
...
# @progress for nplus in 1:10:nplusmax
    ...
    pred_ar = AGP.predict_ar(m, p, nplus, y_past = _ys)
