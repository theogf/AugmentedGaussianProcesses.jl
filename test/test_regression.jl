...
t_full = @elapsed fullmodel = OMGP.GPRegression(X,y,γ=noise,kernel=kernel,VerboseLevel=3)
t_sparse = @elapsed sparsemodel = OMGP.SparseGPRegression(X,y,Stochastic=false,Autotuning=false,VerboseLevel=3,m=20,γ=noise,kernel=kernel)
t_stoch = @elapsed stochmodel = OMGP.OMGP.SparseGPRegression(X,y,Stochastic=true,BatchSize=20,Autotuning=true,VerboseLevel=2,m=20,γ=noise,kernel=kernel)
