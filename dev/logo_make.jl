...
grid = collect(range(-1,9.5,length=500))
...
ygrid = range(-0.3,0.3,length=50)


ksig = 0.2
# fgrid = f.(pgreen,siggreen)
Ygreen = hcat([f(mu,ksig*sigma).(ygrid) for (mu,sigma) in zip(pgreen,siggreen)]...)
Yred = hcat([f(mu,ksig*sigma).(ygrid) for (mu,sigma) in zip(pred,sigred)]...)
Ypurple = hcat([f(mu,ksig*sigma).(ygrid) for (mu,sigma) in zip(ppurple,sigpurple)]...)
Xgrid = hcat([j for i in grid, j in ygrid][:],[i for i in grid, j in ygrid][:])

scene = Scene(backgroundcolor=:white)
image!(grid,ygrid,RGBA.(green,alpha*Ygreen'))
image!(grid,ygrid,RGBA.(red,alpha*Yred'))
image!(grid,ygrid,RGBA.(purple,alpha*Ypurple'))

plot!(grid,pgreen,color=green,linewidth=lw)
...
# image(eachrow(Xgrid)...,color=RGBA.(green,Ygreen[:]))
