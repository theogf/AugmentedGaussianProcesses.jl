...
function writegeneral(file::JLD2.JLDFile,model::GPModel)
    ...
    write(file,"noise",getvalue(model.noise))
...
function readgeneral(file::JLD2.JLDFile,model::GPModel)
    ...
    model.noise = HyperParameter{Float64}(Float64(read(file,"noise")),KernelModule.interval(OpenBound{Float64}(zero(Float64)),NullBound{Float64}()))
