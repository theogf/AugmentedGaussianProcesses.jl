function ∇L_ρ_reverse(f,gp,X)
    Zygote.gradient(()->_∇L_ρ_reverse(f,gp.kernel,first(gp.σ_k),X),Flux.params(gp.kernel)).grads # Zygote gradient
end

_∇L_ρ_reverse(f,kernel,σ,X) = f(σ*kernelmatrix(kernel,X,obsdim=1))

function ∇L_ρ_reverse(f,gp::_SVGP,X,∇E_μ,∇E_Σ,i,opt)
    Zygote.gradient(()->_∇L_ρ_reverse(f,gp.kernel,first(gp.σ_k),gp.Z.Z,X,∇E_μ,∇E_Σ,i,opt),Flux.params(gp.kernel)).grads # Zygote gradient
end
...
function ∇L_ρ_reverse(f,gp::_OSVGP,X,∇E_μ,∇E_Σ,i,opt)
    Zygote.gradient(()->_∇L_ρ_reverse(f,gp.kernel,first(gp.σ_k),gp.Z.Z,X,gp.Zₐ,∇E_μ,∇E_Σ,i,opt),Flux.params(gp.kernel)).grads # Zygote gradient
end
...
function Z_gradient_reverse(gp::_SVGP{T},f_Z::Function,X,∇E_μ::AbstractVector{T},∇E_Σ::AbstractVector{T},i::Inference,opt::AbstractOptimizer) where {T<:Real}
    return first(Zygote.gradient(()->_Z_gradient_reverse(f,gp.kernel,first(gp.σ_k),gp.Z.Z,X,∇E_μ,∇E_Σ,i,opt),Flux.params(gp.Z.Z)).grads) # Zygote gradient
end
...
function Z_gradient_reverse(gp::_OSVGP{T},f_Z::Function,X,∇E_μ::AbstractVector{T},∇E_Σ::AbstractVector{T},i::Inference,opt::AbstractOptimizer) where {T<:Real}
    ...
    return Zygote.gradient(()->_Z_gradient_reverse(f_Z,gp.kernel,first(gp.σ_k),gp.Z.Z,X,gp.Zₐ,∇E_μ,∇E_Σ,i,opt),p).grads[first(p)] # Zygote gradient
end
