function ∇L_ρ_reverse(f, gp::Abstract_GP, X)
    k = gp.kernel
    ps = Flux.params(k)
...
function ∇L_ρ_reverse(f, gp::_SVGP, X, ∇E_μ, ∇E_Σ, i, opt)
    global g = Zygote.gradient(
        () -> _∇L_ρ_reverse(f, gp.kernel, gp.Z.Z, X, ∇E_μ, ∇E_Σ, i, opt),
        Flux.params(gp.kernel),
...
function ∇L_ρ_reverse(f, gp::_OSVGP, X, ∇E_μ, ∇E_Σ, i, opt)
    Zygote.gradient(
        () -> _∇L_ρ_reverse(f, gp.kernel, gp.Z.Z, X, gp.Zₐ, ∇E_μ, ∇E_Σ, i, opt),
        Flux.params(gp.kernel),
    ).grads # Zygote gradient
end
...
function Z_gradient_reverse(
    ...
    return Zygote.gradient(
        () ->
            _Z_gradient_reverse(f_Z, gp.kernel, gp.Z.Z, X, ∇E_μ, ∇E_Σ, i, opt),
        Flux.params(gp.Z.Z),
    ).grads[gp.Z.Z] # Zygote gradient
end
