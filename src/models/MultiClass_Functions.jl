function variablesUpdate_MultiClass!(model::MultiClass,iter)
    expec_f2 = broadcast((var,m)->sqrt.(var.+m.^2),diag.(model.ζ[model.KIndices]),model.μ[model.KIndices])
    ...
    model.γ = broadcast((f,μ)->1.0./(2.0*model.β.*cosh.(0.5.*f)).*exp.(digamma.(model.α).-0.5.*μ),expec_f2,model.μ[model.KIndices])
    model.α = [1+sum(broadcast(x->x[i],model.γ[model.KIndices])) for i in 1:model.nSamples]
    model.θ[2:end] = broadcast((γ,f)->0.5.*γ./f.*tanh.(0.5.*f),model.γ[model.KIndices],expec_f2)
    (model.η_1, model.η_2) = naturalGradientELBO_MultiClass(model.Y,model.θ[1],model.θ[2:end],model.invK,model.γ[model.KIndices])
...
function ELBO(model::SparseMultiClass)
    ...
    ELBO_v += 0.5*sum(broadcast((y,gam,mu,theta,sigma,kappa)->logdet(model.invK)+logdet(sigma)+dot(y-gam,kappa*mu)-sum((kappa'*Diagonal(y.*model.θ[1]+theta)*kappa+invK).*(sigma+mu*(mu'))),broadcast(x->x[model.MBIndices],model.Y),broadcast(x->x[model.MBIndices],model.γ),model.μ,broadcast(x->x[model.MBIndices],model.θ[2:end]),model.ζ,model.κ))
    ELBO_v += sum(broadcast((gam,f2,theta)->dot(gam,-log(2)-log.(gam)+1+digamma.(model.α)-log.(model.β))-sum(log.(cosh.(0.5*f2)))+0.5*dot(f2,f2.*theta),model.γ,expec_f2,model.θ[2:end]))
    ELBO_v += sum([-log.(cosh.(0.5*expec_f2[model.y_class[i]][i]))+0.5*model.θ[1][i]*(expec_f2[model.y_class[i]][i]^2) for i in 1:model.nSamples])
    return -ELBO_v
end
