...
function ∇L_ρ(f,gp::_SVGP,X,∇E_μ,∇E_Σ,i,opt)
    Zygote.gradient(()->_∇L_ρ(f,gp.kernel,first(gp.σ_k),gp.Z.Z,X,∇E_μ,∇E_Σ,i,opt),Flux.params(gp.kernel))
end
...
function ∇L_ρ(f,gp::_OSVGP,X,∇E_μ,∇E_Σ,i,opt)
    Zygote.gradient(()->_∇L_ρ(f,gp.kernel,first(gp.σ_k),gp.Z.Z,X,gp.Zₐ,∇E_μ,∇E_Σ,i,opt),Flux.params(gp.kernel))
end

## Gradient ersatz for OSVGP ##
function _∇L_ρ(f,kernel,σ,Z,X,Zₐ,∇E_μ,∇E_Σ,i,opt)
    Kmm = σ*kernelmatrix(kernel,Z,obsdim=1)
    Knm = σ*kernelmatrix(kernel,X,Z,obsdim=1)
    Knn = σ*diag(kernelmatrix(kernel,X,obsdim=1)) # TO FIX ONCE Zygote#429 is fixed.
