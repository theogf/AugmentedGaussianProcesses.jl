...
"""
mutable struct SoftMaxLikelihood{T<:Real,A<:AbstractVector{T}} <:
    ...
    function SoftMaxLikelihood{T}(
        ...
        new{T, Vector{T}}(Y, class_mapping, ind_mapping, y_class)
    end
end

SoftMaxLikelihood(nClasses::Int) = SoftMaxLikelihood{Float64}(nClasses)
SoftMaxLikelihood(ylabels::AbstractVector) = SoftMaxLikelihood{Float64}(
    length(ylabels),
    ylabels,
...
function grad_samples(
    model::AbstractGP{T,<:SoftMaxLikelihood},
...
function hessian_softmax(s::AbstractVector{<:Real}, i::Integer)
    ...
    for j = 1:m
        for k = 1:m
            hessian[j, k] =
                s[i] *
                ((δ(i, k) - s[k]) * (δ(i, j) - s[j]) - s[j] * (δ(j, k) - s[k]))
        end
    end
    return hessian
end
