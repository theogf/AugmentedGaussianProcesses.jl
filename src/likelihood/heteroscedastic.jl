...
struct HeteroscedasticLikelihood{T<:Real} <: RegressionLikelihood{T}
    ...
    function HeteroscedasticLikelihood{T}(kernel::AbstractVector{<:Kernel},μ₀::AbstractVector{<:PriorMean},λ::AbstractVector{<:Vector{T}},c::AbstractVector{<:AbstractVector{T}},γ::AbstractVector{<:AbstractVector{T}},θ::AbstractVector{<:AbstractVector{T}},μ::AbstractVector{<:AbstractVector{T}},Σ::AbstractVector{<:Symmetric{T,Matrix{T}}},K::AbstractVector{<:Symmetric{T,Matrix{T}}},invK::AbstractVector{<:Symmetric{T,Matrix{T}}},λσg::AbstractVector{<:AbstractVector{T}}) where {T<:Real}
...
function init_likelihood(likelihood::HeteroscedasticLikelihood{T},inference::Inference{T},nLatent::Integer,nSamplesUsed::Integer) where {T<:Real}
    ...
    λ = [ones(T,nSamplesUsed) for _ in 1:nLatent]
    ...
    λσg = [ones(T,nSamplesUsed) for _ in 1:nLatent]
...
function local_updates!(model::VGP{HeteroscedasticLikelihood{T}}) where {T<:Real}
    model.likelihood.λ .= broadcast((μ,Σ,y)->0.5*(abs2.(μ-y)+Σ),model.μ,diag.(model.Σ),model.y)
...
function ∇μ(model::VGP{HeteroscedasticLikelihood{T},AnalyticVI{T}}) where {T<:Real}
    return 0.5*hadamard.(model.y,model.likelihood.λσg)
end
