...
```
Augmentation is described later (#TODO)
"""
struct HeteroscedasticLikelihood{T<:Real} <: RegressionLikelihood{T}
    ...
    λσg::LatentArray{Vector{T}}
    function HeteroscedasticLikelihood{T}(kernel::Union{Kernel,AbstractVector{<:Kernel{T}}}=RBFKernel(),priormean::PriorMean=ConstantMean(0.0)) where {T<:Real}
    ...
    function HeteroscedasticLikelihood{T}(kernel::AbstractVector{<:Kernel},μ₀::AbstractVector{<:PriorMean},λ::AbstractVector{T},c::AbstractVector{<:AbstractVector{T}},γ::AbstractVector{<:AbstractVector{T}},θ::AbstractVector{<:AbstractVector{T}},μ::AbstractVector{<:AbstractVector{T}},Σ::AbstractVector{<:Symmetric{T,Matrix{T}}},K::AbstractVector{<:Symmetric{T,Matrix{T}}},invK::AbstractVector{<:Symmetric{T,Matrix{T}}},λσg::AbstractVector{<:AbstractVector{T}}) where {T<:Real}
         new{T}(kernel,μ₀,λ,c,γ,θ,μ,Σ,K,invK,λσg)
     end
end

function HeteroscedasticLikelihood() where {T<:Real}
    HeteroscedasticLikelihood{Float64}()
end
...
function init_likelihood(likelihood::HeteroscedasticLikelihood{T},inference::Inference{T},nLatent::Integer,nSamplesUsed::Integer) where {T<:Real}
    ...
    λσg = [zeros(T,nSamplesUsed) for _ in 1:nLatent]
    HeteroscedasticLikelihood{T}(kernel,μ₀,λ,c,γ,θ,μ,Σ,K,invK,λσg)
end
...
function cond_mean(model::VGP{HeteroscedasticLikelihood{T},AnalyticVI{T}},index::Integer) where {T<:Real}
    return model.y[index].*model.likelihood.λσg[index]
end

function ∇μ(model::VGP{HeteroscedasticLikelihood{T},AnalyticVI{T}}) where {T<:Real}
    return hadamard.(model.y,model.likelihood.λσg)
end

function cond_mean(model::SVGP{HeteroscedasticLikelihood{T},AnalyticVI{T}},index::Integer) where {T<:Real}
    return model.y[index][model.inference.MBIndices].*model.likelihood.λσg[index]
end

function ∇μ(model::SVGP{HeteroscedasticLikelihood{T},AnalyticVI{T}}) where {T<:Real}
    return hadamard.(getindex.(model.y,[model.inference.MBIndices]),model.likelihood.λσg)
end

function ∇Σ(model::SVGP{HeteroscedasticLikelihood{T},AnalyticVI{T}}) where {T<:Real}
    return model.likelihood.λσg
end

function proba_y(model::VGP{HeteroscedasticLikelihood{T},Analytic{T}},X_test::AbstractMatrix{T}) where {T<:Real}
    μf, σ²f = predict_f(model,X_test,covf=true)
    μg, σ²g = _predict_f.(model.likelihood.μ,model.likelihood.Σ,model.likelihood.invK,model.likelihood.kernel,X_test,[model.X],covf=true)
    return μf,σ²f.+broadcast((λ,μ,σ)->λ*expectation.(logistic,Normal.(μ,sqrt.(σ))),model.likelihood.λ,μg,σ²g)
end
