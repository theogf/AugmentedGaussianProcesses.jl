...
function sample_parameters(
    ...
    init_sampler!(inference(m), nLatent(m), nFeatures(m), nSamples, cat_samples)
    computeMatrices!(m)
    for i in 1:(inference(m).nBurnin + nSamples * inference(m).thinning)
        ...
        if nIter(inference(m)) > m.inference.nBurnin &&
           ((nIter(inference(m)) - m.inference.nBurnin) % (inference(m).thinning) == 0) # Store variables every thinning
            store_variables!(inference(m), means(m))
        end
    end
    symbols = ["f_" * string(i) for i in 1:nFeatures(m)]
    if nLatent(m) == 1
        return Chains(
            reshape(m.inference.sample_store[:, :, 1], :, nFeatures(m), 1), symbols
        )
    else
        return [
            Chains(reshape(m.inference.sample_store[:, :, i], :, nFeatures(m), 1), symbols)
...
function post_process!(m::MCGP{T}) where {T}
